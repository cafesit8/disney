        ...
        <article className='ml-[5%] max-w-[800px] mr-[5%] flex flex-col gap-2'>
            ...
            <div className="flex gap-4">
                {movieDetails.genres?.map(genre => (
                    <span className="font-medium text-gray-300 text-lg font-sans" key={genre.id}>{genre.name}</span>
                ))}
            </div>
            <p className='text-[17px]'>{movieDetails.overview}</p>
            <p>Popularity: <span className="text-gray-300">{movieDetails.popularity}</span></p>
            <p>Status <span className="text-gray-300">{movieDetails.status}</span></p>
