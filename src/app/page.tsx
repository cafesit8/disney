...
async function getAsideDataTv(){
  try {
    ...
    if (response.ok) {
      const result = await response.json();
      const list: Card[] = result.results.filter((item: Card) => item.backdrop_path !== null && item.overview !== '')
...
export default async function HomePage() {
  ...
  const asideData = await getAsideDataTv();
  ...
  return (
    <>
      ...
      <GridMovies asideData={asideData!} movies={data} />
    </>
  )
}
